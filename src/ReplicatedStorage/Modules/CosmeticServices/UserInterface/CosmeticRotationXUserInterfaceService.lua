...
local RotationXIcon_upvr = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("CosmeticUI"):WaitForChild("RotationXIcon")
local ActualIcon_upvr = RotationXIcon_upvr:WaitForChild("ActualIcon")
RotationXIcon_upvr.Visible = false
local module_upvr = {
	Active = false;
	CurrentOffset = 0;
}
...
local function _() -- Line 27, Named "UpdatePosition"
	...
	RotationXIcon_upvr.Position = UDim2.fromOffset(any_GetMouseLocation_result1.X + any_GetGuiInset_result1.X, any_GetMouseLocation_result1.Y - any_GetGuiInset_result1.Y)
end
function module_upvr.Toggle(arg1, arg2) -- Line 34
	--[[ Upvalues[6]:
		...
		[5]: RotationXIcon_upvr (readonly)
		[6]: ActualIcon_upvr (readonly)
	]]
	module_upvr.CurrentOffset = 0
	module_upvr.Active = arg2
	var7_upvw = UserInputService_upvr:GetMouseLocation().Y
	...
	RotationXIcon_upvr.Position = UDim2.fromOffset(any_GetMouseLocation_result1_2.X + any_GetGuiInset_result1_2.X, any_GetMouseLocation_result1_2.Y - any_GetGuiInset_result1_2.Y)
	ActualIcon_upvr.Position = UDim2.fromScale(0.5, 0.5)
	RotationXIcon_upvr.Visible = arg2
end
...
game:GetService("RunService").RenderStepped:Connect(function() -- Line 49
	...
	if not module_upvr.Active then
		...
		local Y = UserInputService_upvr:GetMouseLocation().Y
		local var16 = module_upvr
		var16.CurrentOffset += Y - var7_upvw
		var7_upvw = Y
		ActualIcon_upvr.Position = UDim2.fromScale(0.5, 0.5) + UDim2.fromOffset(0, RoundToNearestNumber_upvr(module_upvr.CurrentOffset, ROTATION_CONFIG_upvr.ROTATION_ROUNDING_PIXEL))
	end
end)
local tbl_upvr = {
